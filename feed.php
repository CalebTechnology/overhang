...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    <main>
      <div class="jumbotron">
        <div class="container">
          <h1 class="display-5">Latest Post</h1>
          <hr>
          <?php
             include 'includes/connect.php';
             $sql = "SELECT * FROM feed ORDER BY id DESC LIMIT 1";
             foreach ($conn->query($sql) as $row) {
               $datetime = date_create($row['uploaded']);
               $display_date = date_format($datetime, "M d Y g:i:s A");
               echo "<div class='container'>" . PHP_EOL;
               echo "<h3 class='display-5'>" . $row['title'] . " by " . $row['username'] . "</h3>" . PHP_EOL;
               echo "<h6>" . $display_date . PHP_EOL;
               if($row['edited'] == true){
                 echo " (edited)";
               }
               echo "</h6>";
               echo "<div class='container'><img src='" . $row['image'] . "' class='img-fluid'></div>" . PHP_EOL;
          ...
          ?>
          <hr>
          <a href='dashboard/new_post.php'><button type="button" name="button" class="btn btn-lg btn-primary btn-block">New Post</button></a>
        </div>
      </div>
      <!--Old News-->
      <div class="container">
        ...
        <div class="container">
          <?php
             include 'includes/connect.php';
             $sql = "SELECT * FROM feed WHERE id < (SELECT MAX(id) FROM feed) ORDER BY id DESC";
             foreach ($conn->query($sql) as $row) {
               $datetime = date_create($row['uploaded']);
               $display_date = date_format($datetime, "M d Y g:i:s A");
               echo "<div class='card container'><p></p>";
               echo "<h4>" . $row['title'] . " by " . $row['username'] . "</h4>" . PHP_EOL;
